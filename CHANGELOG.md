...
- 🔓 **Device permanently stuck on "Starting kiosk…" after reboot with a secure lock screen** (#222): On a Device Owner device that also has a native secure lock (PIN/pattern/password), FreeKiosk could hang forever on the loading screen after a reboot if `BootLockActivity` started before the user unlocked. Root cause: `BootLockActivity`'s `SHOW_WHEN_LOCKED` / `DISMISS_KEYGUARD` flags **occlude** (do not dismiss) a *secure* keyguard, so the user can't enter their credential, credential-encrypted (CE) storage never unlocks, and `MainActivity` (not `directBootAware`) can never launch: the retry loop spins forever. The opt-in "System screen-lock compatibility" (#199) already avoids this by deferring at boot, but it is off by default, so a fresh secure device still deadlocked. Added a **reactive safety net**: if `BootLockActivity` is still unable to hand off after ~8s **and** detects it is sitting over a secure keyguard with the user still locked (`KeyguardManager.isDeviceSecure()` + `UserManager.isUserUnlocked()`), it finishes so the keyguard is revealed; the user unlocks and the kiosk starts cleanly at `BOOT_COMPLETED`. It is **reactive** (only fires on a proven stall) and gated on a real secure lock, so it never affects the normal non-secure fast-boot path: a normal boot hands off first, and a non-secure device never enters the branch. This also covers the secure-lock cases still reported on the broader "Starting kiosk…" hang (#176) after the beta.2 CE-retry fix. Reported by @potofcoffee.
