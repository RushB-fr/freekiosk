...
public class RNCWebViewClient extends WebViewClient {
    ...
    public void onReceivedSslError(final WebView webView, final SslErrorHandler handler, final SslError error) {
        ...
        // ✅ Check if certificate is already accepted
        String certFingerprint = getCertificateFingerprint(error.getCertificate());
        if (isCertificateAccepted(webView.getContext(), certFingerprint) && 
            code == SslError.SSL_UNTRUSTED && 
            topWindowUrl.equalsIgnoreCase(failingUrl)) {
            // Certificate already trusted by user
        ...
        if (code == SslError.SSL_UNTRUSTED && topWindowUrl.equalsIgnoreCase(failingUrl)) {
            ...
            builder.setMessage("This server's SSL certificate is not trusted.\n\n" +
                    "Certificate: " + error.getCertificate().getIssuedTo().getDName() + "\n" +
                    "URL: " + failingUrl + "\n\n" +
            ...
            builder.setPositiveButton("Proceed & Remember", (dialog, which) -> {
                ...
                // ✅ Save the certificate as accepted
                saveCertificateAcceptance(webView.getContext(), certFingerprint);
                handler.proceed();
            });
    ...
    // ✅ Helper methods to manage certificate acceptance

    /**
     * Get certificate fingerprint (unique identifier)
     * Simple version using Issuer + Subject
     */
    private String getCertificateFingerprint(android.net.http.SslCertificate cert) {
        try {
            // Use issuer + subject as unique identifier
     ...
     */
    private boolean isCertificateAccepted(Context context, String fingerprint) {
        SharedPreferences prefs = context.getSharedPreferences(
            "freekiosk_ssl_certs", 
            Context.MODE_PRIVATE
        );
        return prefs.getBoolean("cert_" + fingerprint.hashCode(), false);
    }

    /**
     * Save certificate as accepted by user
     */
    private void saveCertificateAcceptance(Context context, String fingerprint) {
        SharedPreferences prefs = context.getSharedPreferences(
            "freekiosk_ssl_certs", 
            Context.MODE_PRIVATE
        );
        prefs.edit()
            .putBoolean("cert_" + fingerprint.hashCode(), true)
            .apply();
        Log.i(TAG, "Certificate saved as accepted: " + fingerprint);
    }

    /**
     * Clear all accepted certificates (call this when resetting settings)
     */
